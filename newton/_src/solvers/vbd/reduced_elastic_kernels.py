...
def _accumulate_elastic_contact_modes(
    ...
    joint_q: wp.array[float],
    elastic_mode_block_grad: wp.array[float],
    elastic_mode_block_matrix: wp.array[float],
):
...
def assemble_elastic_joints(
    ...
    joint_q: wp.array[float],
    elastic_mode_block_grad: wp.array[float],
    elastic_mode_block_delta: wp.array[float],
    elastic_mode_block_matrix: wp.array[float],
):
    elastic_index = wp.tid()
    body = elastic_body[elastic_index]
    ...
    for endpoint_slot in range(endpoint_slot_count):
        ...
        lam_lin = joint_lambda_lin[joint]
        C0_lin_v = joint_C0_lin[joint]
        ...
        if k_lin > 0.0:
            ...
            C_stab = C - avbd_alpha * C0_lin_v
            PC = P * C_stab
            joint_force = k_lin * PC + P * lam_lin
            h_scale = k_lin
            if kd_lin > 0.0:
                dC_dt = (C - C_prev) * inv_dt
                joint_force = joint_force + kd_lin * (P * dC_dt)
                h_scale = h_scale + (kd_lin * inv_dt)
        ...
        if jt == JointType.FIXED or jt == JointType.PRISMATIC or jt == JointType.REVOLUTE:
            ...
            if jt == JointType.REVOLUTE:
                ...
                drive_tau, drive_H_aa = evaluate_revolute_drive_limit_force_hessian(
                    ...
                    joint_target_q[joint_target_q_start[joint]],
                    joint_target_qd[dof_idx],
...
def solve_elastic_body(
    ...
    elastic_mode_block_delta: wp.array[float],
    elastic_mode_block_matrix: wp.array[float],
    elastic_mode_block_initial_residual_norm: wp.array[float],
    elastic_mode_block_solve_residual_norm: wp.array[float],
    elastic_mode_block_applied_residual_norm: wp.array[float],
    elastic_mode_block_update_norm: wp.array[float],
    elastic_mode_block_update_max: wp.array[float],
