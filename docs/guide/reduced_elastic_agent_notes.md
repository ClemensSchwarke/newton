...
## Curved Blade Workflow (Compliant ANYmal)

- `ModalGeneratorCurvedBeam` builds a reduced elastic basis for a curved
  leaf-spring blade from a planar centerline (`*_centerline.npy`) plus
  `width`/`depth`/`young_modulus`: 3D Euler-Bernoulli beam, clamped-root modes,
  exact `psi` from the beam rotation DoFs. Math in
  `reports/reduced_elastic_curved_beam_modal_solve.tex`.
- Asset: `generate_compliant_urdf_no_shanks.py` emits ANYmal with the compliant
  shank blade removed (the leg ends at the `*_SHANK` knee-output link); the blade
  is rebuilt in Newton from the centerline.
- Frame mapping (checked against the n10 asset): the blade lies in `*_SHANK`-local
  `x = -0.03` and bends about `x`; `(cx, cy) -> (-0.03, root_y + s*cx, cy)` with
  `root_y = +0.05, s = -1` (left) and `-0.05, +1` (right). Place the frame at the
  `*_SHANK` world pose (probe `eval_fk`), attach one fixed joint at `root_local`
  (clamped-root, so `phi = 0` there), and refresh `body_q` before the solver.
- Contact/render: attach `surface_mesh()` as the collision shape
  (`compute_inertia=False`); it also renders, so no foot body is needed. Its
  vertices are the basis corner samples, so `phi` is exact.
- Collision-filter the blade against the robot's own shapes. Its root vertices
  overlap the leg and otherwise fire ~120 spurious self-contacts that detonate the
  solve; the USD uses `enable_self_collisions=False`, so exclude the blade the
  same way (`add_shape_collision_filter_pair` per blade/robot pair), leaving ~8-10
  ground contacts. Diagnose explosions via `rigid_contact_count` vs body height
  (many contacts while high = self-overlap). Stabilizers:
  `elastic_contact_relaxation=0.30`, `rigid_contact_k_start=ke`, `kd ~0.35`.
- `young_modulus` is the calibration knob (`6.9e10` Pa ~ the PRDM chain's tuned
  `6000` tip stiffness); calibrate against Abaqus. No CI/README entry (external
  assets, like `example_robot_anymal_compliant`); generator tested by
  `test_modal_generator_curved_beam_cantilever`.
